...
- `README.md` 是主要阅读入口；`snapshot/` 保存脱敏、可审计的机器清单。
- 仅记录可公开的配置结构、能力名称、来源、状态和版本。
- 保持改动聚焦；未经用户明确要求，不安装、升级或删除 Skill、插件、MCP 服务、依赖或本地工具。

## 更新快照

1. 先运行 `./scripts/check-snapshot.sh` 识别已有漂移，再按 `README.md` 的“更新环境快照”一节执行只读盘点。
2. 先核实完整现状，再修改文档与 `snapshot/`；不得根据缓存目录、当前会话能力或记忆推断启用状态。
